...
Standard library only, so there is nothing to install:
    ...
    python3 scripts/ascore_cli.py
    python3 scripts/ascore_cli.py --server http://other-machine:8080
...
# what "help" prints: one line per command, in the order they are defined below
HELP = {
    "health": "health : is AS-CORE up, and can it reach asdb and Redis",
    "snapshot": "snapshot : the whole current state (nodes, queue, active jobs, config, alerts)",
    "metrics": "metrics : only the shayveri_ lines from /actuator/prometheus",
    "audit": "audit [action] : dashboard changes in the last 24h, e.g. audit config.activate",
    "telemetry": "telemetry <placeId> [players] : send one snapshot as a Roblox server would",
    "register": "register <nodeId> [blender] [slots] : become a node, and start heartbeating every 10s",
    "beat": "beat on|off : start or stop the background heartbeat. off = the node dies",
    "claim": "claim [blender] : ask for a job. Waits up to 20s; 204 means nothing to do",
    "progress": "progress <pct> [log text] : report progress on the job you hold",
    "complete": "complete [resultRef] : finish the job you hold",
    "fail": "fail [error] : fail the job you hold; AS-CORE retries it until maxRetries",
    "nodes": "nodes : every node and whether it is UP or DOWN",
    "create": "create <TYPE> <mapId> [priority] : queue a job. TYPE: TEXTURE_BAKE LIGHT_BAKE MESH_OPTIMIZE PATHFIND_PRECOMPUTE CUSTOM",
    "jobs": "jobs [STATUS] : list jobs, optionally only QUEUED, CLAIMED, RUNNING, DONE or FAILED",
    "save": "save <namespace> key=value ... [place=<id>] : save a new config version, e.g. save spawns zombieSpeed=16",
    "activate": "activate <namespace> <version> [placeId] : make a version live. Older version = rollback",
    "active": "active [placeId] : poll config as Roblox would. Sends the last ETag, so unchanged = 304",
    "history": "history [placeId] : every saved config version",
    "quit": "quit : exit. The heartbeat stops with it, so a registered node goes DOWN",
}
...
class Cli(cmd.Cmd):
    ...
    def do_active(self, arg):
        ...
        status, headers, _ = self.request("GET", "/api/config/active" + (f"?placeId={arg}" if arg else ""),
                                                "roblox", headers={"If-None-Match": etag} if etag else None)
    ...
    def do_help(self, arg):
        # help [command] : one command's line, or all of them
        if arg:
            return print(HELP.get(arg, f"no command {arg}"))

        for line in HELP.values():
            print(line)
